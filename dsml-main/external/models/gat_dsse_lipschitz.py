...
logger = logging.getLogger("GAT_DSSE_Lipschitz")

class GAT_DSSE_Lipschitz_Lightning(pl.LightningModule):
    def __init__(self, hyperparameters, x_mean, x_std, edge_mean, edge_std, reg_coefs, time_info = True, loss_type='gsp_wls', loss_kwargs=None, lipschitz_k=1.0):
        super().__init__()
        ...
        # Initialize GNN model with Lipschitz constraint
        self.model = GAT_DSSE_Lipschitz(dim_feat=dim_feat, dim_hidden= dim_hidden, dim_dense=dim_dense, dim_out=dim_out,
                              num_layers=num_layers, edge_dim=edge_dim, heads=heads, dropout=dropout, lipschitz_k=lipschitz_k)
...
class GAT_DSSE_Lipschitz(nn.Module):
    def __init__(self, dim_feat, dim_hidden, dim_dense, dim_out, num_layers, edge_dim, heads =1, concat = True, slope = 0.2, self_loops = True, dropout = 0., nonlin = 'leaky_relu', model = 'gat', lipschitz_k=1.0):
        super().__init__()
        ...
        self.gat_layers.append(
            GATv2Conv(in_channels=self.dim_feat, out_channels=self.dim_hidden,
        ...
        for l in range(self.num_layers-1):
            self.gat_layers.append(
                GATv2Conv(in_channels=self.dim_hidden, out_channels=self.dim_hidden,
                         heads=self.heads, concat=self.concat, negative_slope=self.slope,
                         dropout=self.dropout, add_self_loops=self.loop, edge_dim=self.edge_dim)
            )

        # Dense layers
        self.dense1 = Linear(in_features=self.dim_hidden, out_features=self.dim_dense)
        self.dense2 = Linear(in_features=self.dim_dense, out_features=self.dim_out)

        # Apply Lipschitz constraint to linear layers
        self._apply_lipschitz_constraint()

    def _apply_lipschitz_constraint(self):
        """Apply spectral normalization to enforce Lipschitz constraint."""
        with torch.no_grad():
            ...
            for layer in [self.dense1, self.dense2]:
                if hasattr(layer, 'weight'):
                    ...
                    # Scale weights if largest singular value exceeds k
                    if s[0] > self.lipschitz_k:
                        layer.weight.data = weight * (self.lipschitz_k / s[0])
            ...
            for gat_layer in self.gat_layers:
                for name, param in gat_layer.named_parameters():
                    if 'weight' in name and param.dim() >= 2:
                        ...
                        # Scale weights if largest singular value exceeds k
                        if s[0] > self.lipschitz_k:
                            param.data = param.data * (self.lipschitz_k / s[0])

    def forward(self, x, edge_index, edge_attr):
        # Apply Lipschitz constraint before forward pass
        self._apply_lipschitz_constraint()
